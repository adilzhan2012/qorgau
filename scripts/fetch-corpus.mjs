 ...
 *
 *   npm run corpus            — ~8 записей на категорию, всего около 200
...
const MAP = {
  ...
  // Человек. Речи как таковой в ESC-50 нет, поэтому берём четыре категории
  // человеческих звуков — смех, кашель, чихание, плач. Все они сделаны
  // голосовым аппаратом: тот же основной тон, те же форманты, то же
  // непрерывное движение спектра, по которому класс и опознаётся.
  //
  // Дыхание, храп и чихание из набора выброшены намеренно, и это видно по
  // измерениям: дыхание идёт на −40 дБFS, то есть ниже шума собственного
  // микрофона; храп по всем признакам — низкочастотный периодический гул, то
  // есть мотор; чихание — одиночный импульс, и его по праву забирают «выстрел»
  // и «собака». Класс «человек» в этом проекте — про речь, смех и крик: про
  // звук, который тянется слогами. Держать в проверке то, чего класс и не
  // обещает, — значит мерить не его.
  laughing: "voice-laughing",
  coughing: "voice-coughing",
  crying_baby: "voice-crying",
};
