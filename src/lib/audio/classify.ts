...
/** Громкость события над фоном парка, 0..1: −45 дБFS тихо, −12 громко. */
function loudness(rms: number): number {
  return ramp(rms, -45, -12);
}
...
function score(w: WindowFeatures): Terms {
  ...
  return {
    ...
    chainsaw: [
      ...
      { label: "громко", weight: 2.0, value: loud },
    ...
    // Фон. Постоянный небольшой уровень, чтобы «ничего не подошло» не
    // превращалось в случайный класс, плюс явная награда за тишину.
    other: [
      // «Ничего заметного» — это не просто тихо, это ещё и без событий. Окно,
      // в котором громкость скакнула на тридцать децибел, заметным быть
      // обязано: без этого условия тихий далёкий лай проигрывал тишине.
      { label: "ничего не происходит", weight: 2.5, value: 1 - ramp(w.levelSpan, 0.35, 0.75) },
      { label: "фоновый уровень", weight: 2.0, value: 0.42 },
      { label: "тихо", weight: 3.0, value: 1 - loud },
    ],
  };
}
